...
CellMatRecord Mulc(CellMatRecord le,CellMatRecord ri) := TRANSFORM
		SELF.cellMat := $.Mul_Mat_Num_2 (ri.cellMat,-1);

		SELF := le;
	END;
	
  J := JOIN(Result,Result2,LEFT.id=RIGHT.id,Mulc(LEFT,RIGHT)); // Form all of the intermediate computations
