//this version is a newer version of optimization_new where I have avoided loops inside the wolfelinesearch_4_4
...
EXPORT Optimization_new_new_2 (UNSIGNED4 prows=0, UNSIGNED4 pcols=0, UNSIGNED4 Maxrows=0, UNSIGNED4 Maxcols=0) := MODULE
...
//LBFGS algorithm
  ...
  ENDC++;
    ...
    END;
		...
		lbfgsUpdate ( DATASET(minfRec) oldsy, DATASET(Layout_Part) y,DATASET(Layout_Part) s , REAL8 ys, INTEGER8 itr_num_) := FUNCTION
    ...
    END; // END lbfgsUpdate
		...
		lbfgs (DATASET(minfRec) min_in, UNSIGNED itr_n) := FUNCTION
...
lbfgs_2 (DATASET(minfRec) min_in, UNSIGNED itr_n) := FUNCTION
      k := IF (itr_n >corrections, corrections, itr_n); // k is the number of previous step vectors which are already stored
      //calculate q and al
      //inp has the previous q value with no=1 and the al values start from no+1 to no+k+1 which correponds to al[1] to al[k], each al[i] is calculated in one iteration from i=k to i=1
		SumProduct_ro (DATASET(minfRec) inp1, DATASET(minfRec) inp2) := FUNCTION
    ...
    END;//END SumProduct_ro_
			q_step_ (DATASET(minfRec) inp_, unsigned4 coun) := FUNCTION
				...
				new_q := PB_daxpy_min(-1*al_, min_in(no=y_ind), inp);
				al_min := DATASET([{1,1,1,1,1,1,1,1,[1],s_ind+1,0,0,0,0,al_}],minfRec);// al_ values is stored in sty field
        RETURN min_in(no=y_ind); // check whether funnel changes the distribution
      END; //END q_step_
			topass_q_ := min_in (no=1);
			topass_q := PB_dscal_min (-1 ,topass_q_);
			q_ := LOOP(topass_q, COUNTER <= 1, q_step_(ROWS(LEFT),COUNTER));
			al_tmp_ := q_(no>1);
      //q_tmp : no=1 has the q vector, n=2 to n=k+1 have al values corespondant ro al[1] tp al[k]
    ...
    //inp includes : no[1 k] is old_dirs matrix or s. no[k+1 2*k] is old steps or y, no[2*k+1] is g from previous iteration, no [2*k+2] is x from previous iteration
     ...
     //RETURN FinalResult; orig
      ...
      f_pre := f_table[1].f;
      d_lbfgs := lbfgs (inp, c);// orig Hdiag should be defined
			hrec := RECORD 
        REAL8 h ;//hdiag value
      END;
      ...
      //gtd := Extractvalue(PBblas.PB_dgemm(TRUE, FALSE, 1.0,param_map, g_pre,param_map, d,one_map )); orig
			SumProduct_min_d := FUNCTION
				...
				prod := JOIN(inp(no=1), d, LEFT.partition_id = RIGHT.partition_id, hp(LEFT,RIGHT), LEFT OUTER, LOCAL);
				RETURN SUM(prod, prod.v);
			END;
      ...
      // w_g := wolfe_g (w);
			 ...
			 w := WolfeLineSearch4_4(1, x_pre,CostFunc_params,TrainData, TrainLabel,CostFunc, param_num, t_init, d, f_pre, g_pre,gtd, 0.0001, 0.9, 3, 0.000000001);//orig ws 25 instead of 3
      ...
      y_s := SumProduct (y_y, s_s);
			...
			sy_updated := lbfgsUpdate ( inp(no>2), y_y, s_s,  y_s, c);
      ...
      RETURN FinalResult;
		...
		min_step_2 (DATASET(minfRec) inp,unsigned4 c) := FUNCTION
      g_pre_ind := 1;
      x_pre_ind := 2;
      inp_ :=  PROJECT(inp, TRANSFORM(PBblas.Types.MUElement, SELF:=LEFT),LOCAL);
      g_pre := PROJECT(inp(no=g_pre_ind), TRANSFORM(layout_part, SELF:=LEFT),LOCAL); // This is the g calculated in the preceding iteration
...
//		RETURN a2;
a4:= LOOP(min_result_firstitr,  5 ,min_step(ROWS(LEFT),COUNTER));
 //RETURN LOOP(min_result_firstitr,  1 ,min_step(ROWS(LEFT),COUNTER));
 RETURN LOOP(min_result_firstitr,  400 ,min_step(ROWS(LEFT),COUNTER));
 //RETURN CostGrad;
// RETURN a1;
