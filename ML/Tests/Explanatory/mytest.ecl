...
// SA_mine4_1 :=DeepLearning.Sparse_Autoencoder_lbfgs (f,hl,392,15298,392,15298);
...
//OUTPUT(IntW1 + IntW2+ IntB1 + IntB2);
//OUTPUT(lbfgs_model_mine4_1);
...
b2vecmap := PBblas.Matrix_Map(64, 1, 32, 1);
b2vecdist := DMAT.Converted.FromElement(Intb2,b2vecmap);

w1map := PBblas.Matrix_Map(5, 8, 5, 2);
w1dist := DMAT.Converted.FromElement(IntW1,w1map);

PBblas.Types.value_t sp_reci(PBblas.Types.value_t v,PBblas.Types.dimension_t r,PBblas.Types.dimension_t c) := v+10;
 aa := PBblas.Apply2Elements(b2vecmap, b2vecdist, sp_reci);
 bb :=  PBblas.Apply2Elements(b2vecmap, PBblas.MU.TO( b2vecdist,2), sp_reci);
//OUTPUT(b2vecdist);
al := PROJECT(b2vecdist, TRANSFORM(PBblas.Types.Layout_Part, SELF.partition_id := STD.System.Thorlib.Node(); SELF := LEFT), LOCAL);
//OUTPUT(al);
//OUTPUT(bb);
// OUTPUT(w1dist);
// OUTPUT(IntW1);
...
h := myfunc5(w1dist,emptyC,emptyL,emptyL);
...
// END;
// x1 := DATASET([{1,4,0},{2,3,0},{3,5,0},{4,6,0},{5,7,0},{6,9,0}], myrec );
// x1_d := DISTRIBUTE (x1,id);

// x2 := DATASET([{1,40,0},{2,30,0},{3,50,0},{4,60,0},{5,70,0},{6,90,0}], myrec );
// x2_d := DISTRIBUTE (x2,id);

// newd := x1_d & x2_d;

// x1_d_p := PROJECT(newd, TRANSFORM(myrec, SELF.nodeid := STD.System.Thorlib.Node(); SELF := LEFT), LOCAL);


// OUTPUT(x1_d_p);
// sy_pre := DATASET([],PBblas.Types.MUElement);
// OUTPUT(STD.System.Thorlib.Node());