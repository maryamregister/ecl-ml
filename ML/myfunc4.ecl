...
emptyL := DATASET([], Layout_Part);
 EXPORT myfunc4 ( DATASET(Layout_Part) x, DATASET(Types.NumericField) CostFunc_params=emptyC, DATASET(Layout_Part) TrainData=emptyL , DATASET(Layout_Part) TrainLabel=emptyL) := FUNCTION
p := 5;
xmap := PBblas.Matrix_Map(p,1,3,1);
