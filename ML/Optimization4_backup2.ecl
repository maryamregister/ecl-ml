...
SHARED WolfeOutput_Record := RECORD
 ...
 extr_which_cond (DATASET(bracketing_record4) i) := FUNCTION
  ...
  BracketingResult :=  LOOP(Topassbracketing, extr_which_cond(ROWS(LEFT))=-1 AND  COUNTER <maxLS  , bracketingstep(ROWS(LEFT),COUNTER)); 
  ...
  ZoomingResult := IF (extr_loop_term(Topass_zooming)=TRUE, Topass_zooming , LOOP(Topass_zooming, extr_loop_term(ROWS(LEFT))=FALSE AND  COUNTER <=Zoom_Max_Itr  , zoomingstep(ROWS(LEFT),COUNTER)));
...
/*
 ...
 min_step_firstitr := FUNCTION
    ...
    min_step (DATASET(minfRec) inp,unsigned4 c) := FUNCTION
      ...
      f_pre := f_table[1].f;
     // d_lbfgs := lbfgs_4 (d_steap, s_pre, y_pre, hDiag_pre); orig // orig Hdiga should be defined 
      d_lbfgs := PBblas.PB_dscal(hDiag_pre,d_steap);
    ...
    min_result := LOOP(min_result_firstitr, COUNTER <1 , min_step(ROWS(LEFT),COUNTER)); 
    RETURN min_result; 
