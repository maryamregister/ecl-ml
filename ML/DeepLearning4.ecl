  ...
  SHARED SA(DATASET(Types.NumericField) X, DATASET(Mat.Types.Element) IntW1, DATASET(Mat.Types.Element) IntW2, DATASET(Mat.Types.Element) Intb1,  DATASET(Mat.Types.Element) Intb2, REAL8 BETA=0.1, REAL8 sparsityParam=0.1 , REAL8 LAMBDA=0.001, REAL8 ALPHA=0.1, UNSIGNED2 MaxIter=100) := MODULE
   ...
   //Map for Matrix d.
    ...
    SparseParam_CostGradients4 (DATASET(Layout_Part) theta, DATASET(ML.Types.NumericField) parameters, DATASET(Layout_Part) train_d, DATASET(Layout_Part) train_l = emptyL) :=  FUNCTION
      //first of all extract the parameters needed in sparse autoencoder from the parameters input
      
      //w1m := w1dist;
      w1m := theta (partition_id <= w1_partitions);
      //w2m := w2dist;
      w2m := theta (partition_id > w1_partitions AND partition_id <= w1_partitions + w2_partitions);
      //b1v := b1vecdist;
      b1v := theta (partition_id > w1_partitions + w2_partitions AND partition_id <= w1_partitions + w2_partitions + b1_partitions);
      //b2v := b2vecdist;
      b2v := theta (partition_id > w1_partitions + w2_partitions + b1_partitions AND partition_id <= w1_partitions + w2_partitions + b1_partitions + b2_partitions);
      a2 := FF2 (w1m, b1v);
      a3 := FF3 (w2m, b2v, a2);
      d3 := DELTA3 (a3);
      rohat_a2 := rohat(a2);
      ...
      PBblas.Types.MUElement addpart(Layout_Part l, INTEGER8 c ) := TRANSFORM
