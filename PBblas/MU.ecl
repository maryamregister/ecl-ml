...
EXPORT To(DATASET(Types.Layout_Part) d, Types.t_mu_no num) := PROJECT(d, TRANSFORM(Types.MUElement, SELF.no := num, SELF := LEFT),LOCAL);
EXPORT From(DATASET(Types.MUElement) d, Types.t_mu_no num) := PROJECT(d(no=num), TRANSFORM(Types.Layout_Part, SELF := LEFT),LOCAL);
